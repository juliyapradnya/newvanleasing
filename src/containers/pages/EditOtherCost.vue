  ...
  >
    ...
    <b-form
      ...
      <b-form-group :label="$t('additional.vendor')"  class="has-top-label">
        ...
        />
        <b-form-invalid-feedback v-if="!$v.vendor_name.required">Vendor name is required!</b-form-invalid-feedback>
      </b-form-group>
      <b-form-group :label="$t('additional.amount')" class="has-top-label">
        <b-input-group>
          <money v-model="$v.amount_oc.$model" v-bind="money" class="form-control" :state="!$v.amount_oc.$error"></money>
        </b-input-group>
    ...
    <template slot="modal-footer">
      <b-button variant="outline-secondary" @click="hideModal('editothercost')">{{ $t('pages.cancel') }}</b-button>
      <b-button
        :class="{
        ...
        variant="primary" type="submit" form="editCostForm"
...
export default {
  ...
  methods: {
    ...
    getEditData(items) {
      this.$refs.editothercost.show();
      this.editId = items.id
      this.date = items.date
      this.id_purchase_order = items.vehicle_registration
      this.description_expenses = items.description_expenses
      this.amount_oc = items.amount_oc
      this.vendor_name = items.vendor_name
      this.tempPurchaseId = items.id_purchase_order
    },
    onEditFormSubmit() {
      let url = apiUrl + "/othercost/" + this.editId;
